...
<title>TrendFrend Risk 0.5 Clean</title>
...
.top{{display:flex;justify-content:space-between;align-items:start;margin-bottom:20px}}
.title h2{{font-size:42px;margin:0 0 7px}}.title p{{margin:0;color:#9fb4c9;font-size:16px}}
.badge{{font-size:13px;color:var(--green);border:1px solid rgba(48,209,88,.4);border-radius:999px;padding:5px 10px;margin-left:10px}}
.pill{{border:1px solid var(--line);border-radius:999px;padding:12px 18px;background:rgba(255,255,255,.06)}}
.grid{{display:grid;gap:14px}}
.kpis{{grid-template-columns:repeat(8,1fr);margin-bottom:14px}}
.charts{{grid-template-columns:1fr 1fr;margin-bottom:14px}}
.lower{{grid-template-columns:1fr 1fr;margin-bottom:14px}}
.bottom{{grid-template-columns:1.35fr .65fr}}
...
.label{{color:#c7c7cc;font-size:13px}}.value{{font-size:25px;font-weight:850;letter-spacing:-.03em;margin:8px 0 5px}}.sub{{color:var(--muted);font-size:13px}}
...
.mini-grid{{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}}
.mini{{border:1px solid var(--line);border-radius:14px;padding:13px;background:rgba(0,0,0,.15)}}.mini b{{display:block;font-size:22px;margin:6px 0}}
...
<div class="app">
<aside>
<div class="brand"><div class="logo">T</div><div><h1>TrendFrend</h1><p>Risk 0.5 Clean</p></div></div>
<nav><a class="active">⌘ Обзор</a><a>↗ Производительность</a><a>▦ Помесячно</a><a>◎ Инструменты</a><a>⚙ Настройки</a></nav>
<div class="side-card"><div class="label">Режим</div><h3>Risk 0.5% / month</h3><p>Monthly Fixed-Return<br><b>Compounding</b></p></div>
<div class="side-card"><div class="label">Сравнение</div><h3>DD снижена ×2</h3><p>1.0% risk: <b>26.04%</b></p><p>0.5% risk: <b>13.79%</b></p></div>
</aside>
<main>
<div class="top"><div class="title"><h2>TrendFrend <span class="badge">RISK 0.5% CLEAN</span></h2><p>Monthly Fixed-Return Compounding • данные только из out_risk_05pct_monthly</p></div><div class="pill">СТАТИЧЕСКАЯ АНАЛИТИКА</div></div>
...
<div class="card"><div class="section"><h3>Помесячная сводка</h3><span>Risk 0.5%</span></div><div class="mini-grid" id="monthSummary"></div></div>
...
<table style="margin-top:14px"><thead><tr><th>Инструмент</th><th>Доходность</th><th>Доля</th><th>Сделок</th><th>Win / Loss</th></tr></thead><tbody id="instTable"></tbody></table>
</div>
</div>
</main>
</div>
...
const sci=v=>Number(v).toExponential(2).replace('e+',' × 10^');
...
kpis.innerHTML=[
kpi('Доходность',sci(totalRet)+'%','summary.json','green'),
kpi('MaxDD',fmt(maxDD)+'%','month-to-month','red'),
kpi('Profit Factor',fmt(pf,4),'summary.json','green'),
kpi('Win Rate',fmt(wr)+'%','summary.json'),
kpi('Средняя сделка',plus(avgTrade)+fmt(avgTrade,3)+'%','avg_trade_return'),
kpi('Среднее время',fmt(avgHold,2)+' ч','avg_hold_hours'),
kpi('Всего сделок',fmt(trades,0),m.length+' месяцев'),
kpi('Сделок в месяц',fmt(avgTradesMonth,1),'средняя активность')
].join('');

eqNote.textContent='Final Equity: '+sci(num(eq.at(-1).equity ?? eq.at(-1).end_eq));
...
mini('Средний месяц',plus(avgMonthly)+fmt(avgMonthly)+'%','avg_monthly_return','green'),
...
(OUT / "risk05-clean.html").write_text(html, encoding="utf-8")
print("built:", OUT / "risk05-clean.html")
