...
DST = Path("/home/ubuntu/.hermes/bots/champion_live_bot/public_report/data/baseline_v2026_07")
...
for p in [MONTHLY, SUMMARY, TRADES]:
    if not p.exists():
...
# monthly report: 2021-01 | Start=1000.00 | Return=159.52% | PnL=1595.21 | End=2595.21 | Trades=534 | WR=47.57%
monthly = []
for raw in MONTHLY.read_text(encoding="utf-8", errors="ignore").splitlines():
    line = raw.strip()
    m = re.match(
    ...
    monthly.append({
        "month": month,
        "start": round(num(start), 2),
        "return_pct": round(num(ret), 2),
        "pnl": round(num(pnl), 2),
        "end": round(num(end), 2),
        "trades": int(num(trades)),
        "win_rate": round(num(wr), 2),
    })

if not monthly:
    raise SystemExit("ERROR: parsed 0 monthly rows")

# summary csv
with SUMMARY.open(newline="", encoding="utf-8-sig") as f:
    sr = list(csv.DictReader(f))[0]
sr = {k.lower(): v for k, v in sr.items()}
...
instrument = defaultdict(lambda: {"pnl": 0.0, "trades": 0, "wins": 0})
instrument_month = defaultdict(lambda: {"pnl": 0.0, "trades": 0, "wins": 0})
exit_types = Counter()

tp_count = sl_count = reverse_count = tp2_count = 0
...
for r in trades:
    ...
    instrument[symbol]["pnl"] += pnl
    instrument[symbol]["trades"] += 1
    instrument[symbol]["wins"] += int(pnl > 0)
...
symbol_summary = []
for s in symbols:
    x = instrument.get(s, {"pnl": 0.0, "trades": 0, "wins": 0})
...
summary = {
    ...
    "total_return_pct_summary": round(num(sr.get("total_return_pct")), 2),
    "total_return_pct_monthly_fixed_return_compounding": round((monthly[-1]["end"] / monthly[0]["start"] - 1) * 100, 2),
    "profit_factor": round(num(sr.get("profit_factor")), 4),
    "win_rate": round(num(sr.get("win_rate_pct")), 2),
    "max_drawdown_pct": round(num(sr.get("max_dd_pct")), 2),
    "total_trades": int(num(sr.get("total_trades"))),
...
if summary["total_trades"] <= 0 or summary["profit_factor"] <= 0 or len(monthly) < 12:
    raise SystemExit("ERROR: invalid generated data")
...
payloads = {
    ...
    "symbol_summary.json": symbol_summary,
}

for name, obj in payloads.items():
    (DST / name).write_text(json.dumps(obj, ensure_ascii=False, indent=2))
