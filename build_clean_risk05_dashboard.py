...
.label{{color:#c7c7cc;font-size:13px}}.value{{font-size:26px;font-weight:850;letter-spacing:-.03em;margin:8px 0 5px}}.sub{{color:var(--muted);font-size:13px}}.green{{color:var(--green)}}.red{{color:var(--red)}}
...
<div class="side-card"><div class="label">Главное отличие</div><h3>DD снижена ×2</h3><p>26.04% → 13.79%</p><p>PF ≈ <b>2.04</b></p></div>
</aside>
<main>
...
<div class="card"><div class="section"><h3>Помесячная сводка</h3><span>только Risk 0.5%</span></div><div class="mini-grid" id="monthSummary"></div></div>
...
<div class="card">
<div class="section"><h3>Помесячная доходность (%)</h3><span>данные из monthly.csv</span></div>
...
<div class="card">
<div class="section"><h3>Инструменты</h3><span>данные из instrument_summary.csv</span></div>
<div class="pie-stack">
...
function line(id,labels,data,color,fill,ddScale=false){{
 new Chart(document.getElementById(id),{{type:'line',data:{{labels,datasets:[{{data,borderColor:color,backgroundColor:fill,borderWidth:2,tension:.22,pointRadius:0,fill:true}}]}},options:{{responsive:true,maintainAspectRatio:false,plugins:{{legend:{{display:false}}}},scales:{{x:{{grid:{{display:false}},ticks:{{color:'#8b8f98',maxTicksLimit:8}}}},y:ddScale?{{min:-100,max:0,grid:{{color:'rgba(255,255,255,.08)'}},ticks:{{color:'#8b8f98',stepSize:10,callback:v=>fmt(v,0)+'%'}}}}:{{grid:{{color:'rgba(255,255,255,.08)'}},ticks:{{color:'#8b8f98'}}}}}}}}}});
}}
...
kpis.innerHTML=[
kpi('Доходность',sci(totalRet)+'%','из summary.json','green'),
kpi('MaxDD',fmt(maxDD)+'%','month-to-month','red'),
kpi('Profit Factor',fmt(pf,4),'из summary.json','green'),
kpi('Win Rate',fmt(wr)+'%','из summary.json'),
...
(OUT / "risk05.html").write_text(html, encoding="utf-8")
print("BUILT CLEAN:", OUT / "risk05.html")
