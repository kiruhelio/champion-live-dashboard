...
def score_file(p):
    s = str(p).lower()
    score = 0
    if "out/best_candidate" in s: score += 1000
    if "best_candidate" in s: score += 700
    if "3m" in s: score += 400
    if "1h" in s: score += 400
    if "trade" in p.name.lower(): score += 300
    if "audit_075_results" in s: score -= 1000
    if "public_report/data" in s: score -= 2000
    return score

manual = os.environ.get("DASH_SRC")
if manual:
    SRC = Path(manual)
else:
    files = [p for p in ROOT.rglob("*.csv") if p.is_file() and "trade" in p.name.lower()]
    files = sorted(files, key=lambda p: (score_file(p), p.stat().st_mtime), reverse=True)
    print("TOP CSV CANDIDATES:")
    for p in files[:20]:
        print(score_file(p), p)
    if not files:
        raise SystemExit("ERROR: no trades CSV found")
    SRC = files[0]
...
if not date_col:
    for c in cols:
        if any(re.search(r"20\d\d[-/]\d\d", str(r.get(c,""))) for r in rows[:100]):
            date_col = c
            break
...
for name, obj in {
    ...
    "monthly.json": monthly_rows,
    "yearly.json": yearly_rows,
    "symbols.json": symbol_rows,
    "equity.json": equity,
